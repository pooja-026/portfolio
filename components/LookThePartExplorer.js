...
import { ArrowRight, BarChart3, Database, Play, ShieldCheck, X } from "lucide-react";
...
export default function LookThePartExplorer({ open, onClose }) {
  ...
  return <div className="case-study-backdrop" role="dialog" aria-modal="true" aria-label="LookThePart project case study">
    <section className="case-study-panel">
      ...
      <div className="journey-explorer"><div><p className="case-kicker">Interactive route</p><h3>From a post to a product trail.</h3><p>Tap each stage to trace the product-discovery path that the analytics layer captures.</p></div><div className="journey-steps"><button><span>01</span> Post ID</button><ArrowRight /><button><span>02</span> Exact match</button><ArrowRight /><button><span>03</span> Similar looks</button><ArrowRight /><button><span>04</span> Retailer</button><ArrowRight /><button><span>05</span> Related products</button></div></div>
      <div className="case-map"><img src="/lookthepart/journey-map.png" alt="LookThePart user journey map from post ID through product discovery" /><p>Journey mapping artifact: post IDs connect exact and similar matches, retailers, and related product choices.</p></div>
